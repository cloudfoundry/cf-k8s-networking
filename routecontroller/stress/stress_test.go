...
var _ = Describe("Stress Tests", func() {
	...
	It(fmt.Sprintf("does not get more than %d%% worse", allowableDeltaPercent), func() {
		stressRouteController(numberOfRoutes)
...
func stressRouteController(numberOfRoutes int) {
	fmt.Println("Adding 1000 routes at once")
	setupRoutes(numberOfRoutes, "bulk")
